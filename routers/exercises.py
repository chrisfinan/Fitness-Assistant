...
# Helper function to map time to an integer
...
async def get_exercises_by_info(
    ...
    # Update or insert user survey info
    ...
    # Primary Exercise Classification (PEC) Mapping
    pec_mapping = {
    ...
    # Calculate max exercises selected
    ...
    # Define selected/queried exercises
    selected_exercises = []

    for group in condition_groups:
        # Filter by results choice
        query = db.query(Exercise).filter(
            Exercise.primary_exercise_classification.in_(pec_mapping[results])
        )

        # Filter by days (conditions) choice
    ...
    # If less than max exercises selected, select more
    ...
    # If more than max exercises, trim off some
    selected_exercises = selected_exercises[:max_exercises]

    # Update Choose table to tie exercises to user
